...
function Tooltip({ alert, pos }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 10, padding: "12px 14px", minWidth: 180, pointerEvents: "none",
      fontFamily: FONTS.sans, fontSize: 12,
      boxShadow: `0 4px 24px rgba(0,0,0,.4)`,
    }}>
      <div style={{ fontWeight: 700, fontFamily: FONTS.mono, color: col, marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
        {alert.type}
        {alert.live && <span style={{ fontSize: 9, padding: "1px 5px", background: "rgba(0,255,136,.12)", color: COLORS.green, borderRadius: 3 }}>LIVE</span>}
      </div>
      ...
      {alert.description && (
        <div style={{ marginTop: 8, fontSize: 10, color: COLORS.muted, fontFamily: FONTS.mono }}>{alert.description}</div>
      )}
    </div>
  );
}
...
export default function MapView({ alerts, loading, selected, onSelect }) {
  ...
  const validAlerts = alerts?.filter(alert => 
    alert && 
    typeof alert.x === 'number' && 
    typeof alert.y === 'number' &&
    !isNaN(alert.x) && 
    !isNaN(alert.y)
  ) || [];
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", background: COLORS.bg }}>
      ...
      <svg width="100%" height="100%" viewBox="0 0 1000 500" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* Incident Markers - Only render valid alerts */}
